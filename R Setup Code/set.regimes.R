set.converge.regimes<-function(trdata,regimes){
  ...
  #Returns a new column for the dat = regimes
  ...
  return(list(trdata$dat$regimes,trdata$phy$node.label))
}

set.regimes<-function(tdata,shifts){
  library(ggsci)
  phy<-trdata$phy
...
#shifts<-c(83,72,65) #Location of nodes with regime shifts #50 species
  new.trdata<-data.frame(phy$tip.label)
  new.trdata<-make.treedata(phy,trdata)
  new.trdata<-set.converge.regimes(trdata,shifts)
  trdata$dat$regimes<-new.trdata[[1]]
  trdata$phy$node.label<-new.trdata[[2]]
  ...
  reg.colors<-ggsci::pal_npg(palette=c("nrc"),alpha=1)(length(unique(shifts.total)))

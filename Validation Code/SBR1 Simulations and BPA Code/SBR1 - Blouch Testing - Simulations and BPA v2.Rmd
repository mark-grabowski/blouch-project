...
cervid.dataset<-read.csv("/Users/markgrabowski/Documents/Academic/Research/Current Projects/Blouch project/R1/Updated Dataset/updated_mean_dat_no_OL_MG.csv")
...
##sim<-stan.adapt.OU.inac.hl(tips.list=NULL,cervid.trdata$phy,N,opt.slope,hl.list,vy,vY0,vX0,theta)
...
#sim<-slouch.adapt.OU.inac.hl(tips.list=NULL,cervid.trdata$phy,N,opt.slope,hl.list,vy,vY0,vX0,theta)
...
#sim<-stan.direct.OU.inac.hl(tips.list=NULL,cervid.trdata$phy,N,direct.slope,hl.list,vy,vY0,vX0,theta)
...
#sim<-slouch.direct.OU.inac.hl(tips.list=NULL,cervid.trdata$phy,N,direct.slope,hl.list,vy,vY0,vX0,theta)
...
```{r}
save.image(file = "SBR1 - Blouch and Slouch - Main Sims - Adaptive and Direct - WS_122222.RData")
...
sigma2y.list<-c(0.05,0.1,0.25,0.3,0.4,0.5)
...
```{r}
save.image(file = "SBR1 - Blouch - Main Sims - BM - WS_122022.RData")
```
...
vY0 <- 2
vX0 <- 4
theta<-4 #Previously 0

##################################
vy<-0.001

sim<-stan.adapt.OU.inac.hl(tips.list=tips.list,tree,N,opt.slope,hl.list,vy,vY0,vX0,theta)
saved.adapt.parameters<-rbind(saved.adapt.parameters,sim[[1]])
saved.adapt.bpa<-rbind(saved.adapt.bpa,sim[[2]])

sim<-stan.adapt.OU.inac.hl(tips.list=tips.list,tree,N,opt.slope,hl.list,vy,vY0,vX0,theta,sym=1)
saved.adapt.parameters<-rbind(saved.adapt.parameters,sim[[1]])
saved.adapt.bpa<-rbind(saved.adapt.bpa,sim[[2]])
...
Direct Effect Model
```{r}
#source("/Uxsers/markgrabowski/Library/CloudStorage/GoogleDrive-mark.walter.grabowski@gmail.com/Other computers/My MacBook Pro (1)/Academic/Research/Current Projects/Blouch project/R1/Blouch Testing - SBR1/Functions/SBR1.stan.OU.adapt.inac.hl.R")
source("/Users/markgrabowski/Documents/Academic/Research/Current Projects/Blouch project/R1/Blouch Testing - SBR1/Functions/SBR1.stan.OU.adapt.inac.hl.R")
...
sim<-stan.direct.OU.inac.hl(tips.list=tips.list,tree,N,direct.slope,hl.list,vy,vY0,vX0,theta)
saved.direct.parameters<-rbind(saved.direct.parameters,sim[[1]])
saved.direct.bpa<-rbind(saved.direct.bpa,sim[[2]])

sim<-stan.direct.OU.inac.hl(tips.list=tips.list,tree,N,direct.slope,hl.list,vy,vY0,vX0,theta,sym=1)
saved.direct.parameters<-rbind(saved.direct.parameters,sim[[1]])
saved.direct.bpa<-rbind(saved.direct.bpa,sim[[2]])
...
```{r}
save.image(file = "SBR1 - Blouch Testing - Main Sims - Changing Tips 122222_WS.RData")
```
...
```{r}
save.image(file = "SBR1 - Blouch and Slouch - BPA Plots - Adaptive and Direct - WS_122222.RData")
...
```{r}
save.image(file = "SBR1 - Blouch - BPA Plots - Blouch BM - WS_122222.RData")

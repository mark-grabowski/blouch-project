---
title: "Blouch Ruminants Prediction Example"
...
Here we will use Blouch to predict either an X or Y trait using inverse or clasical regression. Either way we assume that trait X is the predictor and Y is the response. We will start with Classical Regression, and use the same example dataset from (Toljagić et al. 2017. As We did previously.
...
## Classical Regression
```{r Classical Regression Plot Setup}
...
```{r Make Classical Regression Plot}
...
```{r Show Trace and Density Plots}
par(mfrow = c(1,3))
...
Inverse Regression
```{r Inverse Regression PLot Setup}
old.par <- par(mar = c(0, 0, 0, 0))
par(old.par)

...
brain.predictions<-apply(posterior.fit.fos.direct$X_pred_fos_means,2,mean)
brain.predictions.extant<-apply(posterior.fit.fos.direct$X_pred_extant_means,2,mean)
...
fos.predictions<-data.frame(Genus_Species = ruminant.trdata$phy$tip.label[fos.index], = ruminant.trdata$dat$brain_mass_g_log_mean[fos.index], log_brain_pred = brain.predictions[fos.index],log_body=ruminant.trdata$dat$bodycentered[fos.index],Status="Extinct")
...
theme_bw()+
theme(legend.position="bottom")+ #5X5
xlab("log Brain Mass (g)")+
  ylab("log Neocortex Area (mm2)")

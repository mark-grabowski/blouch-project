...
blouchOU.iter.setup.v1<-function(trdata,names.traits,post.optima){
  ...
  ta[ta==0]<-0.0000000001 #Added because dividing by ta in newest Blouch
  T.term <- times[1:n]
  tia <- times[1:n] - ta
  tja <- t(tia)
  tij <- tja + tia

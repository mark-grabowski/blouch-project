...
vector beta, int n_regimes, int Z, int Z_direct, int Z_random, real a, vector T_term, int Z_me_direct, int Z_me_random){
...
//Set up X matrix
  ...
  if((Z_me_direct!= 0) || (Z_me_random!= 0)){
    V_me = varcov_measurement(N, ta, direct_cov, mv_direct_cov,mv_random_cov, sigma_squared_x, 
    beta, n_regimes, Z, Z_direct, Z_random, a, T_term, Z_me_direct, Z_me_random);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  if((Z_me_direct!= 0) || (Z_me_random!= 0)){
    V_me = varcov_measurement(N, ta, direct_cov, mv_direct_cov,mv_random_cov, sigma_squared_x, beta, n_regimes, Z,
    Z_direct, Z_random, a, T_term,Z_me_direct,Z_me_random);
  }else{
    V_me = rep_matrix(0,N,N);
  }
  V = Vt + V_me + diag_matrix(mv_response);
  //V = Vt + diag_matrix(mv_response);
  beta_opt = inverse(X'*inverse(V)*X)*(X'*inverse(V)*Y); //Hansen et al. 2008

//////////////////////////////////////////////////////////////////////////////////////////////////////
  //Calculate r2 based on constraint or adaptive regression
  pred_mean = X*beta_opt;
  grand_mean = ((rep_vector(1,N))' * inverse(V) * Y) / sum(inverse(V));
  sst = ((Y - grand_mean)' * inverse(V) * (Y - grand_mean));
  sse = ((Y - pred_mean)' * inverse(V) * (Y - pred_mean));

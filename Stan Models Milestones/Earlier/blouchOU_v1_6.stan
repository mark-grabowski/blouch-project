...
//Works with blouchOU.setup.mv_v1_2.R function
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//OU with random covariates
  ...
  //beta_e[1] ~ normal(beta[1],0.1);
  //for(i in 1:Z_direct){
  //  beta_e[i+1] ~ normal(beta[i+1],0.1);
  //  }
  for(i in 1:Z_random){
    //beta_e[i+Z_direct+1] ~ normal(beta[i+Z_direct+1]* rho,0.1);
    beta_e[i] ~ normal(beta[1+Z_direct+i]* rho,0.1); //Only random covariates get evolutionary regressions
    }  
    

  //mu_e = X_e*beta_e;
  //Y ~ multi_normal_cholesky(mu_e , L_V);
}

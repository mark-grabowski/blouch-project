...
//Code addded for trend model
//Works with blouchOU.setup.mv_v1_3.R function
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//Design Matrix Code - Adaptive
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    matrix design_matrix(int N,real a, vector T_term, matrix direct_cov, matrix random_cov, int Z, int Z_direct, int Z_random, int trend){
    matrix[N,Z+1] X;
    ...
    if(trend == 1){
        rhos = to_matrix(T_term ./ 2); //BM Model
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    matrix design_matrix_evol(int N,real a, vector T_term, matrix direct_cov, matrix random_cov, int Z, int Z_direct, int Z_random, int trend){
    matrix[N,Z_direct+1] X_part;
    matrix[N,Z+1] X;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
  matrix varcov_model(int N, matrix tij, matrix tja, matrix ta, matrix random_cov,real sigma2_y, real a, matrix x0, matrix sigma_squared_x, vector beta, vector T_term, int Z, int Z_direct, int Z_random, int trend){
  ...
  if(trend == 1){
    ...
    if(num_elements(random_cov) != 0){
      ...
      else{ //Direct covariates only or just fixed effects
        Vt = sigma2_y * ta;
      }
  }
  else{
...
model {
  ...
  vector[N] rho;
  //Priors
  hl ~ lognormal(-1.5,1); //Tree length = 1, Original Cervidae tree = 17 Ma - simulations
  //hl ~ lognormal(-2.0,1); //Tree length = 1, Original Ruminant tree = 27 Ma
  vy ~ cauchy(0,0.1);
  
  //a ~ lognormal(1.0,1.0); //a = log(2)/half-life
  //sigma2_y ~ cauchy(0,0.1); //Added for SBR1 for multiple random variables
  //////////////////////////////////////////////////////////////////////////////////////////////////////
  if(trend==1){a=0.00001;}
  else{a = log(2)/hl;}
  ...
  if(num_elements(random_cov)!=0){
    ...
    //k = exp(-a * T_term)*Ya+(1-exp(-a * T_term))*b0+(1-exp(-a*T_term)-rho)*beta[2:Z+1]'*brownian_mean[1,1:Z_random]';
  }
  ...
  //Ya ~ uniform(9,10);
  b0 ~ normal(ols_intercept,0.2);

  if(num_elements(random_cov)!=0){
    beta[1] ~ normal(k, 0.4); //Slope originally 0.4
    beta[2:Z+1] ~ normal(ols_slope, 0.2); //Slope originally 0.4
    //k ~ normal(exp(-a * T_term[1])*Ya+(1-exp(-a * T_term[1]))*b0+(1-exp(-a*T_term[1])-rho)*beta[2]*brownian_mean[1,1],0.2);
    //k ~ normal(exp(-a * T_term)*Ya+(1-exp(-a * T_term))*b0+(1-exp(-a*T_term)-rho)*beta[2:Z+1]'*brownian_mean[1,1:Z_random]',0.2); mvcase
    //k ~ normal(b0+(1-rho)*beta[2]*brownian_mean[1,1],0.2);} //Need to change for mv predictors
  }
  else{
    beta[1] ~ normal(ols_intercept, 0.2); //Slope originally 0.4
    beta[2:Z+1] ~ normal(ols_slope, 0.2); //Slope originally 0.4
    //k ~ normal(b0,0.2);
    }
...
//Set up X matrix
  X = design_matrix(N, a,  T_term, direct_cov,random_cov, Z, Z_direct, Z_random, trend);
  //X_e = design_matrix_evol(N, a,  T_term, direct_cov,random_cov, Z, Z_direct, Z_random);
//Set up V matix
  Vt = varcov_model(N,  tij,  tja,  ta, random_cov, sigma2_y,  a, brownian_mean,  sigma_squared_x,  beta,  T_term, Z, Z_direct, Z_random,trend);
...
//Calculate r2 based on constraint or adaptive regression
  ///////////////////////////////////////
  X_opt = design_matrix(N, a,  T_term, direct_cov,random_cov,  Z,  Z_direct,  Z_random, trend);
  Vt_opt = varcov_model(N,  tij,  tja,  ta, random_cov, sigma2_y,  a, brownian_mean,  sigma_squared_x,  beta,  T_term, Z, Z_direct, Z_random,trend);
  if((num_elements(mv_direct_cov)!= 0) || (num_elements(mv_random_cov)!= 0)){
    //print("sum!=0");

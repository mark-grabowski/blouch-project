...
transformed parameters {
  ...
  //print(beta_evol_complete);
  
  //alpha =  beta[1];
  
  //if(sum(direct_cov) != 0){
    //beta_direct = beta[2:Z_direct+1];
  //  }

  //if(sum(random_cov) != 0){
    //beta_evol = beta_evol_complete[Z_direct+2:Z_direct+Z_random+1];
    //beta_optimal = beta[Z_direct+2:Z_direct+Z_random+2];
    //}
  }
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//Model Block
...
//Priors
  ...
  a ~ lognormal(1.0,1.0); //a = log(2)/half-life
  beta[1] ~ normal(ols_intercept, 0.5); //Slope originally 0.4
...
//Calculate r2 based on constraint or adaptive regression
  ...
  if(sum(mv_response)!= 0){
      V_final = Vt_final + V_me_final + diag_matrix(mv_response); //Hansen & Bartoszek (2012) - Eq 10
    }
    else{
        V_final = Vt_final + V_me_final; //Hansen & Bartoszek (2012) - Eq 10
      }
  }else{
    if(sum(mv_response)!= 0){
       V_final = Vt_final + diag_matrix(mv_response);
    }else{V_final = Vt_final;}
  }

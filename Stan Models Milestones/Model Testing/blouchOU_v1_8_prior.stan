...
parameters {
  real <lower = 0> hl;
  //real <lower = 0, upper = variance(Y)*2> vy; //Added to limit the variance based on Kjetil's suggestion
...
//Calculate r2 based on constraint or adaptive regression
  ...
  //real <lower = 0> vy_sim = fabs(cauchy_rng(0,1));

  //real <lower=0> vy_sim = uniform_rng(0,variance(Y)*2);

  vector[Z+1] beta_sim; //OU beta
  vector[Z+1] beta_e_sim; //OU beta
  ...
  //Set up X matrix
  matrix[N,Z+1] X_sim = design_matrix(N, a_sim,  T_term, direct_cov,random_cov, Z, Z_direct, Z_random);
//Set up V matix
  ...
  X_sim = design_matrix_evol(N, a,  T_term, direct_cov,random_cov,  Z,  Z_direct,  Z_random);
  beta_e_sim[1] = beta_sim[1];

  matrix[N,N] L_V_sim = cholesky_decompose(V_sim);
  for (i in 1:100){
    Y_sim[i,] = (multi_normal_cholesky_rng(X_sim*beta_e_sim, L_V_sim))';
  }

functions {
  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    //Blouch Fixed Niches Code - multilevel optima
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////
    //Vt function
  ///////////////////////////////////////////////////////////////////////////////////////////////////////////
  // Define a separate function for computing Vt when there are direct covariates only
matrix compute_Vt_direct(int N, matrix tij, matrix ta, real sigma2_y, real a) {
  return sigma2_y / (2 * a) * ((1 - exp(-2 * a * ta)) .* exp(-a * tij)); //From Hansen (1997)
}

// Define a separate function for computing Vt when there are random covariates
matrix compute_Vt_random(int N, matrix tij, matrix tja, matrix ta, vector T_term, matrix sigma_squared_x, vector beta, real sigma2_y, real a, int Z_direct, int Z_random) {
  vector[N] sigma2s = rep_vector(1.0, N);
  matrix[N,N] term0 = ((beta[2+Z_direct:1+Z_direct+Z_random]' * sigma_squared_x * rep_vector(1.0, Z_random) + sigma2_y) / (2 * a)) * (1 - exp(-2 * a * ta)) .* exp(-a * tij);
  matrix[N,N] ti = rep_matrix(T_term,N);
  matrix[N,N] term1 = (1 - exp(-a * ti)) ./ (a * ti); 
  matrix[N,N] term2 = exp(-a * tja) .* term1;

  matrix[N,N] Vt = term0 + beta[1+Z_direct+Z_random]^2 * (ta .* term1 .* term1 - ((1 - exp(-a * ta)) ./ a) .* (term2 + term2'));
  return Vt; //From Hansen et al. (2008)
}

// Define the main function
matrix varcov_model(int N, matrix tij, matrix tja, matrix ta, matrix random_cov, real sigma2_y, real a, matrix x0, matrix sigma_squared_x, vector beta, vector T_term, int n_regimes, int Z, int Z_direct, int Z_random) {
  if (sum(random_cov) != 0) {
    return compute_Vt_random(N, tij, tja, ta, T_term, sigma_squared_x, beta, sigma2_y, a, Z_direct, Z_random);
  } else {
    return compute_Vt_direct(N, tij, ta, sigma2_y, a);
  }
}
...
parameters {
  ...
  real <lower = 0> vy;
  //vector[n_regimes+Z] beta;
  vector[Z + n_regimes] beta_raw; //OU beta
  vector[Z_random] beta_e; //OU beta
  real beta_bar; //added for multilevel model
  real <lower=0> sd_beta; //added for multilevel model
...
transformed parameters {
  vector[n_regimes+Z] beta;
  // implies: beta ~ normal(beta_bar, sd_beta)
  beta[1:n_regimes] = beta_bar + sd_beta * beta_raw[1:n_regimes];
  beta[n_regimes+1:n_regimes+Z] = beta_raw[n_regimes+1:n_regimes+Z];
  ...
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    model {
      ...
      //sigma2_y ~ cauchy(0,0.1); //Added for SBR1 for multiple random variables
      
      //beta[1:n_regimes] ~ normal(beta_bar,sd_beta); //Added for SBR1, multilevel model
      beta_raw[1:n_regimes] ~ student_t(2,0,1); //Added for SBR1, multilevel model
      //beta_raw[1:n_regimes] ~ normal(0,0.5); //Added for SBR1, multilevel model
      ...
      beta_bar ~ normal(ols_intercept,0.25); //Added for multilevel model - 3 regimes Prior
      //beta_bar ~ normal(ols_intercept,0.25); //Added for multilevel model - 5 regimes Prior
      //beta_bar ~ normal(ols_intercept,0.25); //Added for multilevel model - 7 regimes Prior

      //beta_bar ~ student_t(2,0,1); //Added for multilevel model - Mean standardized Y
      sd_beta ~ exponential(3); //Added for multilevel model - 3 regimes prior
      //sd_beta ~ exponential(3); //Added for multilevel model - 5 regimes prior
      //sd_beta ~ exponential(3); //Added for multilevel model - 7 regimes prior

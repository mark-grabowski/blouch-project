...
functions {
  ...
  matrix calc_optima_matrix(int N, int n_reg, real a, array[] real t_beginning, array[] real t_end, array[] real times, array[] real reg_match, int[] nodes){
...
data {
  ...
  matrix[N,N] tij;
  array[N, max_node_num,T] real t_beginning; //Array of times for beginning of segments
  array[N, max_node_num, T] real t_end; //Matrix of times for end of segments
  array[N, max_node_num, T] real times; //Matrix of root to node times
  array[N, max_node_num, T] real reg_match; //Matrix of 1,2,3 denoting each regime for each node in a lineage. 0 if no node
...
model {
  ...
  hl_bar ~ normal(0,1);
  hl_sigma ~ exponential(1);
  for(i in 1:T){
    hl ~ normal(hl_bar,hl_sigma);
    vy ~ normal(vy_bar,vy_sigma);

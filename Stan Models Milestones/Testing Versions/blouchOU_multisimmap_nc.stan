...
parameters {
  real<lower=0> hl;
  real<lower=0> vy;
  vector<lower=0>[n_reg] optima_bar;
  real<lower=0> sigma;
  vector[n_reg] z;
}
...
model {
  ...
  for(i in 1:T){
    ...
    sigma ~ exponential(5);
    //sigma ~ normal(0,1);
    dmX = calc_optima_matrix(N, n_reg, a, t_beginning[i,], t_end[i,], times[i,], reg_match[i,], nodes[,i]); //name[i] = accessing tree, not first row of matrix
    mu = dmX*optima;

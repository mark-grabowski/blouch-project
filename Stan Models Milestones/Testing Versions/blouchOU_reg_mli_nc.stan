...
//Multilevel model - varying intercepts/regimes
//Non-centered model
...
data {
  ...
  matrix[N,N] tij;
  matrix[N, max_node_num] t_beginning; //Matrix of times for beginning of segments
  matrix[N, max_node_num] t_end; //Matrix of times for end of segments
  ...
  int nodes[N]; //Vector of number of nodes/segments per lineage
}
...
parameters {
  ...
  real optima_bar;
  vector[n_reg] optima_raw;
  real<lower=0> optima_sigma;
...
transformed parameters{
  ...
  //x~normal(z,exp(v))
  //x = zexp(v)
  vector[n_reg] optima = optima_bar + optima_raw*optima_sigma;
}
...
model {
  ...
  hl ~ lognormal(log(0.4),1.0);
  sigma2_y ~ exponential(5);
  optima_raw ~ normal(0,1);
  optima_bar ~ normal(0,1);
  //optima_sigma ~ exponential(1);
  //Statistical Rethinking p. 408 - use half normal prior when have few clusters/regimes
  optima_sigma ~ normal(0,1);
  //optima ~ normal(optima_bar,optima_sigma);

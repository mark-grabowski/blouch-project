functions {
//////////////////////////////////////////////////////////////////////////////////////////////////////
//Blouch Fixed Niches - Multilevel Model - Varying Effects Code, centered priors
...
///////
 matrix calc_X_matrix(real a, int n_regimes, int n_lineages, int max_node_length, matrix nodes, matrix nodes_time, 
                       matrix t_end, matrix t_beginning, matrix regime_time, int[,] regimes_matrix){
      
      matrix[n_lineages,n_regimes] X_matrix =rep_matrix(0,n_lineages,n_regimes);
      
      for(i in 1:n_lineages){
        int max_j = max_node_length;
        while (max_j > 0 && t_end[i,max_j] == 0) max_j -= 1;
        for(j in 1:max_j){
          //X_matrix[i, regimes_matrix[i,j]] += exp(-a * t_beginning[i,j]) - exp(-a * t_end[i,j]);
          X_matrix[i, regimes_matrix[i,j]] += (1-(1-exp(-a*(t_beginning[i,j]-t_end[i,j]))));
        }
      }
      return(X_matrix);
    }
...
int[,] regimes_matrix, int Z, int Z_direct, int Z_random){
  ...
  X_trait = calc_X_matrix( a,  n_regimes,  n_lineages,  max_node_length,  nodes,  nodes_time,  t_end, t_beginning,  
  regime_time,  regimes_matrix);

...
generated quantities {
  ...
  vector[n_reg,Z_direct+Z_adaptive] beta_sim;
  vector[n_reg+Z_direct+Z_adaptive] optima_beta;
  ...
  vector[N] rho = (1 - (1 - exp(-a * T_term))./(a * T_term));
  for(i in 1:n_reg){
    optima_sim[i] = normal_rng(optima_prior[1],optima_prior[2]);
  }
  for(i in 1:n_reg{
    for(j in 1:(Z_direct+Z_adaptive)){
      beta_sim[i,j]= normal_rng(beta_prior[1],beta_prior[2]);
      }
  }
  optima_beta = append_row(optima_sim,beta_sim);
  ...
  pred_X = calc_mixed_dmX(a,T_term,X_sim,Z_direct,Z_adaptive);//Given measurement error in X variable, uncomment this nested statement
  dmX = append_col(optima_matrix,pred_X);
  V = calc_adaptive_V(a,sigma2_y,ta,tij,tja,T_term, beta_sim[(Z_direct+1):(Z_adaptive+Z_direct)],sigma2_x);

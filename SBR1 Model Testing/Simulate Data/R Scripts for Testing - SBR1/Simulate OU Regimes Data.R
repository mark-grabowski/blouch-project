...
calc_V_SR<-function(Dmat, var_anc, a) {
  N<-dim(Dmat)[1]
...
reg.colors <- pal_aaas("default", alpha = 0.7)(4)
...
sigma2_y<-vy*(2*(log(2)/hl))
#alpha<-4 #Intecept
#optima<-c(2,1.5,1,0.5) #Optima for four regimes#
optima<-c(2,1.5,1,0.5) #Optima for four regimes
dmX<-weight.matrix(trdata$phy, a, lineages) #Slouch approach
mu<-dmX%*%optima #Simulate mu for Y
V<-calc_direct_V(phy, sigma2_y, a)
...
#Goto Simulate OU SIMMAP Data.R
...
#Increasing number of tips to 100 for basic regime model
#Basic Setup
#Vt = sigma2_y /( 2 * a) * ((1 - exp(-2 * a * ta)) .* exp(-a * tij)); //From Hansen (1997) Original Stan
...
#Slouch testing
slouch.results<-slouch.fit(phy = simmap_tree1,
                           species = simmap_tree1$tip.label,
                           response = dat$Y_obs,
...
shifts<-c(54,88) #Location of nodes with regime shifts
trdata<-data.frame(phy$tip.label)
trdata<-make.treedata(phy,trdata)
trdata<-set.converge.regimes(trdata,shifts)
...
sigma2_y<-0.1
alpha<-4 #Intecept
beta<-c(0.35,0.25,0.1) #Slopes for two regimes

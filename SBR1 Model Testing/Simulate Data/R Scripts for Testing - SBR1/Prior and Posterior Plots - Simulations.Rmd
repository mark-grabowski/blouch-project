...
```{r}
calc_adaptive_V_plot<-function(a, sigma2_y, ta,  tij,  tja,  T_term,  beta,  sigma2_x){
  N<-dim(ta)[1];
  Z<-length(beta);
  ts<-ts_fxn(phy)
  ta<-ts[[1]]
...
#############################################################################################################################
Milestone 9: Multilevel Regime Model with direct effect predictors and measurement error
Four regimes
Hl Plot prior vs. posterior - assume posterior has been extracted using extract(model) and stored in post
```{r}
hl.sims<-data.frame(rlnorm(n=1000,meanlog=log(0.25),sdlog=0.75))
...
#############################################################################################################################
Milestone 10: Multilevel Regime Model with Adaptive Predictors and Measurement Error
Hl Plot prior vs. posterior - assume posterior has been extracted using extract(model) and stored in post
```{r}
hl.sims<-data.frame(rlnorm(n=1000,meanlog=log(0.25),sdlog=0.75))
#hl.sims<-data.frame(hl.sims[hl.sims<3])
...
######################################################
Milestone 11: Multilevel Regime Model with direct effect and adaptive Predictors and measurement error
Hl Plot prior vs. posterior - assume posterior has been extracted using extract(model) and stored in post
```{r}
hl.sims<-data.frame(rlnorm(n=1000,meanlog=log(0.25),sdlog=0.75))
#hl.sims<-data.frame(hl.sims[hl.sims<3])
...
plot( NULL , xlim=c(0,1) , ylim=c(0,0.3) , xlab="Time since MRCA" , ylab="Covariance" ,cex.axis=0.75, mgp=c(1.25,0.25,0),tcl=-0.25)
for (i in 1:30){
    curve(calc_adaptive_cov_plot(a.sims[i,],sigma2_y.sims[i,],beta[3],x) , add=TRUE , lwd=4 , col=col.alpha(1,0.15))
}

for (i in 1:30){
    curve(calc_adaptive_cov_plot(post$a[i],post$sigma2_y[i],post$beta[i],x) , add=TRUE , lwd=4 , col=col.alpha(2,0.5))
}
...
ggsave("/Users/markgrabowski/Documents/Academic/Research/Current Projects/Blouch project/blouch - not online/Validation Figures/blouch_validation_figS56.pdf", plot = fig, width=7, height=7 )

pdf(file = "/Users/markgrabowski/Documents/Academic/Research/Current Projects/Blouch project/blouch - not online/Validation Figures/blouch_validation_figS56E.pdf",   # The directory you want to save the file in
    width = 3.6, # The width of the plot in inches
    height = 3.4) # The height of the plot in inches
covariance.plot
dev.off()
...
#############################################################################################################################
Milestone 12: Regime model with direct effect predictors with measurement error and varying slopes
...
#############################################################################################################################
Milestone 13/14: Regime Model with direct effect predictors and varying effects
Hl Plot prior vs. posterior - assume posterior has been extracted using extract(model) and stored in post
```{r}
hl.sims<-data.frame(rlnorm(n=1000,meanlog=log(0.25),sdlog=0.75))
#hl.sims<-data.frame(hl.sims[hl.sims<3])
...
ggsave("/Users/markgrabowski/Documents/Academic/Research/Current Projects/Blouch project/blouch - not online/Validation Figures/blouch_validation_figS62.pdf", plot = fig, width=7, height=7 )

pdf(file = "/Users/markgrabowski/Documents/Academic/Research/Current Projects/Blouch project/blouch - not online/Validation Figures/blouch_validation_figS62C.pdf",   # The directory you want to save the file in
    width = 3.6, # The width of the plot in inches
    height = 3.4) # The height of the plot in inches
covariance.plot
dev.off()
...
#############################################################################################################################
Milestone 15: Regime Model with adaptive model and varying slopes

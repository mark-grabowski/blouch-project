...
//Multilevel regime model with direct and adaptive predictors and varying effects per regime
...
transformed parameters{
  ...
  beta = beta_bar + v[, 2:(1+Z_direct+Z_adaptive)];
...
generated quantities {
  ...
  matrix[n_reg,Z_adaptive] beta_e = beta[,(Z_direct+1):(Z_direct+Z_adaptive)];
  matrix[(1+Z_direct+Z_adaptive),(1+Z_direct+Z_adaptive)] Rho;
  ...
  for(i in 1:N){
      g_i = g_i_v[i];
      c_ii = inverse(V)[i,i];
      log_lik[i] = -0.5*log(2*pi())+0.5*log(c_ii)-0.5*(g_i^2)/c_ii;
      }
}

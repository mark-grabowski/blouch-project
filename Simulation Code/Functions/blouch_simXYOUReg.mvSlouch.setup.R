blouch_simXYOUReg.mvSlouch.setup<-function(trdata,num.direct,num.random,slope,hl,vy,vY0,vX0,reg.values=NULL,reg=NULL,mc=NULL){
  ...
  if(num.direct>0){
    ...
    T.term <- times[1:n]     
    if(reg==TRUE){#Regimes
      reg.matrix<-data.frame(matrix(reg.values,ncol=length(reg.values),nrow=length(num.direct)))
...
#                           Syx=matrix(0,ncol=1,nrow=1),Sxy=matrix(0,ncol=1,nrow=1),starting_point_for_optim=list(A=matrix(A,ncol=1,nrow=1),Syy=matrix(Syy,ncol=1,nrow=1),B=matrix(B,ncol=1,nrow=1),Sxx=matrix(Sxx,ncol=1,nrow=1)))
    ...
    names.direct.traits<-names(OUBMdata)
  }
...
#                           Syx=matrix(0,ncol=1,nrow=1),Sxy=matrix(0,ncol=1,nrow=1),
#                           starting_point_for_optim=list(A=matrix(A,ncol=1,nrow=1),
#                           Syy=matrix(Syy,ncol=1,nrow=1),B=matrix(B,ncol=1,nrow=1),Sxx=matrix(Sxx,ncol=1,nrow=1)))
...
#                          Syy=matrix(Syy,ncol=1,nrow=1),B=matrix(B,ncol=1,nrow=1),Sxx=matrix(Sxx,ncol=1,nrow=1)))
    ...
    names.random.traits<-names(OUBMdata)
    
  }
  OUBMdata<-data.frame(OUBMdata,"tip.names"=row.names(OUBMdata))
  trdata$dat<-cbind(trdata$dat,tip.names=trdata$phy$tip.label)
  OUBMdata<-merge(OUBMdata,trdata$dat,by="tip.names")
  
  trdata <- make.treedata(tree, OUBMdata,name_column="tip.names") #Combine data with tree
  ...
  #return(trdata)
  if(num.direct>0){
    names.direct.traits<-names(trdata$dat)[2:(num.direct+1)]
  }
  if(num.random>0){
    names.random.traits<-names(trdata$dat)[2:(num.random+1)]
  }
  #return(names(trdata$dat))
  ####################################################
  ...
  ############################################################################
  #Fixed regimes
  regimes_internal <-tree$node.label

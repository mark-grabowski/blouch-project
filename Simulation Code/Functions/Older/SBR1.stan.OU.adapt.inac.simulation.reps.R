...
stan.OU.simulation.reps<-function(OUBMparameters,tree,n,opt.slope,evol.slope,hl,vy){
  ...
  for (i in 1:n){
    ...
    fit<- rstan::sampling(object = stan_model,data = stan_data_sub,chains = 1,iter = 1000,verbose=FALSE,control=list(adapt_delta=0.975))

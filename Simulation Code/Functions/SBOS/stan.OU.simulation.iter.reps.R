stan.OU.iter.data.setup<-function(OUBMparameters,tree,n,post.optima){
  source("/Users/markgrabowski/Documents/Academic/Research/Current Projects/Blouch project/Stan Functions/blouch v1/Blouch Setup Files/blouchOU.iter.setup.v1.R")
  #n<-50
  ...
  for(i in 1:n){
    ...
    stan.data[[length(stan.data)+1]]<-blouchOU.iter.setup.v1(OUBM.trdata,names.traits,post.optima)
  }
  return(stan.data)
}

stan.OU.single.data.setup<-function(OUBMparameters,tree,n,post.optima){
  source("/Users/markgrabowski/Documents/Academic/Research/Current Projects/Blouch project/Stan Functions/blouch v1/Blouch Setup Files/blouchOU.iter.setup.v1.R")
  names.traits<-c("trait_1","trait_2",NA,NA)
  OUBMdata<-simulMVSLOUCHProcPhylTree(tree,OUBMparameters,regimes=NULL) #Simulate data
  OUBMdata<-data.frame(OUBMdata,"tip.names"=row.names(OUBMdata))
  OUBM.trdata <- make.treedata(tree, OUBMdata,name_column="tip.names") #Combine data with 
  OUBM.trdata$dat$trait_2<-OUBM.trdata$dat$trait_2-mean(OUBM.trdata$dat$trait_2) #Mean standardize X
  stan.data<-blouchOU.iter.setup.v1(OUBM.trdata,names.traits,post.optima)
  return(stan.data)
}

stan.BM.iter.data.setup<-function(BMparameters,tree,n){
...
stan.OU.simulation.iter.reps<-function(OUBMparameters,tree,n,post.optima){
  setwd("/Users/markgrabowski/Documents/Academic/Research/Current Projects/Blouch project/Stan Functions/blouch v1")
  stanc("/Users/markgrabowski/Documents/Academic/Research/Current Projects/Blouch project/Stan Functions/blouch v1/blouchOU_iter_test.stan")
  stan.model <- "blouchOU_iter_test.stan"
  #######################
  stan.data<-stan.OU.single.data.setup(OUBMparameters,tree,n,post.optima)
  #return(stan.data)
  #######################
  ...
  parameter.names<-c("optimal.reg.median","optimal.reg.mean","optimal.reg.025","optimal.reg.975",
                     ...
                     "t12.median","t12.mean","t12.025","t12.975","sigma2y.median","sigma2y.mean","sigma2y.025","sigma2y.975")
  names(saved.parameters)<-parameter.names
  ###############
  
  for (i in 1:n){
    print(paste("Iteration ",i))
    stan.adaptive.data<-stan.data
    ...
    par.summary <- summary(fit.random, pars = c("beta", "beta_evol","hl","sigma2_y"), probs = c(0.025,0.5, 0.975))$summary
    ...
    print(par.summary[1,2])
    stan.data<-stan.OU.single.data.setup(OUBMparameters,tree,n,par.summary[1,2])
  }

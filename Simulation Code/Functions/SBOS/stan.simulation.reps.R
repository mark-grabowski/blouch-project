...
stan.BM.data.setup<-function(BMparameters,tree,n){
  ...
  stan.data<-list()
  for(i in 1:n){
    BMdata<-simulBMProcPhylTree(tree,BMparameters) #Simulate data
    BMdata<-data.frame(OUBMdata,"tip.names"=row.names(BMdata))
...
stan.OU.simulation.reps<-function(OUBMparameters,tree,n){
  ...
  stanc("/Users/markgrabowski/Documents/Academic/Research/Current Projects/Blouch project/Stan Functions/blouch v1/blouchOU_v1.stan")
  stan.model <- "blouchOU_v1.stan"
  #######################
  stan.data<-stan.OU.data.setup(OUBMparameters,tree,n)
  #######################
  saved.parameters<-data.frame(matrix(NA,n,12))
  parameter.names<-c("optimal.reg.median","optimal.reg.mean","optimal.reg.025","optimal.reg.975","evol.reg.median","evol.reg.mean","evol.reg.025","evol.reg.975","t12.median","t12.mean","t12.025","t12.975")
  ...
  for (i in 1:n){
    ...
    fit.random<- stan(file = stan.model,data = stan.adaptive.data,chains = 2,iter = 4000,control=list(adapt_delta=0.95))
    par.summary <- summary(fit.random, pars = c("beta", "beta_evol","hl"), probs = c(0.025,0.5, 0.975))$summary

#Code to simulate data for regime painting on phylogeny and direct effect and adaptive traits
...
########################################################################################################
#Basic Setup - two regimes with direct and adaptive models
...
dat<-list(N=N,n_reg=length(unique(regimes)),Z_direct=Z_direct,Z_adaptive=Z_adaptive,Y_obs=Y,X_obs=matrix(Xs,nrow=N,ncol=Z),
...
dat<-list(N=N,n_reg=length(unique(regimes)),Z_direct=Z_direct,Z_adaptive=Z_adaptive,Z_X_error=Z_X_error,Y_obs=Y_with_error,
          X_obs=matrix(X_with_error,nrow=N,ncol=Z),Y_error=Y_error,X_error=X_error,
          sigma2_x=sigma2_x,max_node_num=max_node_num,ta=ta,tij=tij,tja=tja,T_term=T_term,t_beginning=t_beginning,
          t_end=t_end,times=times,reg_match=reg_match,nodes=nodes)
...
X_error<-matrix(0.01,nrow=N,ncol=Z_direct)
Y_error<-rep(0.01,N)
Y_with_error<-Y+rnorm(N,0,0.01)
...
optima<-c(2,1.5,1,0.5,0.25) #Optima for four regimes
...
X_error<-matrix(0.01,nrow=N,ncol=Z_direct)
Y_error<-rep(0.01,N)
Y_withdm_error<-Y+rnorm(N,0,0.01)
X_with_error<-X+rnorm(N,0,0.01)
...
#With regime info for each tip species
dat<-list(N=N,n_reg=length(unique(regimes)),Z_direct=Z_direct,Z_X_error=Z_X_error,Y_obs=Y_with_error,X_obs=matrix(X_with_error,nrow=N,ncol=Z_direct),
          Y_error=Y_error,X_error=matrix(X_error,nrow=N,ncol=Z_direct),
          max_node_num=max_node_num,ta=ta,tij=tij,tja=tja,T_term=T_term,t_beginning=t_beginning,
          t_end=t_end,times=times,reg_match=reg_match,nodes=nodes,reg_tips=reg_tips)
...
#Standardize Y and X
#dat<-list(N=N,n_reg=length(unique(regimes)),Z_direct=Z_direct,Z_X_error=Z_X_error,Y_obs=(Y_with_error),X_obs=matrix(standardize(X_with_error),nrow=N,ncol=Z_direct),
#          Y_error=Y_error,X_error=matrix(X_error,nrow=N,ncol=Z_direct),
#          max_node_num=max_node_num,ta=ta,tij=tij,tja=tja,T_term=T_term,t_beginning=t_beginning,
#          t_end=t_end,times=times,reg_match=reg_match,nodes=nodes)
...
dat<-list(N=N,n_reg=length(unique(regimes)),Z_adaptive=Z_adaptive,Y_obs=Y,X_obs=matrix(X,nrow=N,ncol=Z_adaptive),
          sigma2_x=sigma2_x,max_node_num=max_node_num,ta=ta,tij=tij,tja=tja,T_term=T_term,t_beginning=t_beginning,
          t_end=t_end,times=times,reg_match=reg_match,nodes=nodes)
...
dat<-list(N=N,n_reg=length(unique(regimes)),Z_adaptive=Z_adaptive,Z_X_error=Z_X_error,Y_obs=Y_with_error,
          X_obs=matrix(X_with_error,nrow=N,ncol=Z_X_error),Y_error=Y_error,X_error=X_error,
          sigma2_x=sigma2_x,max_node_num=max_node_num,ta=ta,tij=tij,tja=tja,T_term=T_term,t_beginning=t_beginning,
          t_end=t_end,times=times,reg_match=reg_match,nodes=nodes)

blouchsimXYOUReg.setup<-function(trdata,N,num.direct,num.random,slope,hl,vy,vY0,vX0,regimes,direct=0){
  ...
  if(num.direct>0){
    names.direct.traits<-names(data.frame(X))[1:num.direct]  
  }
  if(num.random>0){
    names.random.traits<-names(data.frame(X))[num.direct+1:num.random]
  }
  ...
  if(num.random!=0){
    if(num.random==1){
      ...
      X <- random[,1]
      X_me <- mv.random[,1] 
      brownian[[1]] <-  sigma.X.estimate(phy, ta, X, X_me)
    ...
    if(num.random>1){
      ...
      brownian<-sigma.X.estimate.mv(phy,X)
      #return(brownian)

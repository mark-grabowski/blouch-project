...
##################################################################################################################
#Milestone 14/15
#Two adaptive traits, two regimes 
set.seed(10)
...
print(reg.colors)
plot(trdata$phy,edge.color = reg.colors[edge.regimes], edge.width = 1,show.tip.label=FALSE)
tiplabels(pch=19,cex=2,col=reg.colors[factor(trdata$dat$regimes)])
...
Z_adaptive<-2
sigma2_x<-matrix(c(1,0,0,1),2,2) #No correlation between traits
Xs<-sim.corrs(phy,sigma2_x) #Simulated correlated BM Xs
#Xs<-data.frame(Xs)
...
pred_X<-calc_adaptive_dmX(a,T_term,Xs)
...
dat<-list(N=N,n_reg=length(unique(regimes)),Z_adaptive=Z_adaptive,Y_obs=Y,X_obs=matrix(Xs,nrow=N,ncol=Z_adaptive),
          max_node_num=max_node_num,ta=ta,tij=tij,tja=tja,T_term=T_term,t_beginning=t_beginning,
          t_end=t_end,times=times,reg_match=reg_match,nodes=nodes)
...
X_error<-matrix(0.01,nrow=N,ncol=Z_adaptive)
Y_error<-rep(0.01,N)
Y_with_error<-Y+rnorm(N,0,0.01)

...
Simulate X and Y variable for Single Covariates
```{r}
source("/Users/markgrabowski/Documents/Academic/Research/Current Projects/Blouch project/R1 blouch-testing branch/Simulation Code/Functions/Simulate OU Data.R")
...
#Direct effect model - w/o ME
```{r}
stan_sim_data<-simulate.OU.direct(sampled.tree.10K,hl,vy,slope,vX0,vY0,Sxx=10,theta,num.X = 1)

```

#Adaptive model - w/o ME
```{r}
stan_sim_data<-simulate.OU.adaptive(sampled.tree.10K,hl,vy,slope,vX0,vY0,Sxx=10,theta,num.X = 1)
...
df<-data.frame(Y=stan_sim_data$Y,X=stan_sim_data$random_cov) #Direct effect model
names(df)<-c("Y","X")
ggplot(data=df,aes(x=X,y=Y))+
  geom_point()
lm(Y~X,df)
...
```{r}
tip.label<-sampled.tree.10K$tip.label
Dmat<-cophenetic(sampled.tree.10K)
...
```{r}
#Direct effect model, 1 trait
stan_sim_data<-blouch_simXYOUReg.mvSlouch.setup(trdata,num.direct=1,num.random=0,num.ME.direct=0,num.ME.random=0,slope,hl,vy,vY0,vX0,reg.values=c(4.0,4.25),reg=TRUE,mc=FALSE,sim.errors=FALSE) #Direct effect model

#Direct effect model, 1 trait + ME
stan_sim_data<-blouch_simXYOUReg.mvSlouch.setup(trdata,num.direct=1,num.random=0,num.ME.direct=1,num.ME.random=0,slope,hl,vy,vY0,vX0,reg.values=c(4.0,4.25),reg=TRUE,mc=FALSE,sim.errors =TRUE) #Direct effect model


stan_sim_data<-blouch_simXYOUReg.mvSlouch.setup(trdata,num.direct=0,num.random=1,num.ME.direct=0,num.ME.random=0,slope,hl,vy,vY0,vX0,reg.values=c(4.0,4.25),reg=TRUE,mc=FALSE,sim.errors=FALSE) #Adaptive Nodel no ME

stan_sim_data<-blouch_simXYOUReg.mvSlouch.setup(trdata,num.direct=0,num.random=1,slope,hl,vy,vY0,vX0,reg.values=c(4.0,4.25),reg=FALSE,mc=FALSE) #Adaptive model

```

No regimes
```{r}
stan_sim_data<-blouch_simXYOU.mvSlouch.setup(trdata,num.direct=1,num.random=0,num.ME.direct=0,num.ME.random=0,slope,hl,vy,vY0,vX0,theta,mc=TRUE,sim.errors=FALSE) #Direct Effect Nodel no ME


stan_sim_data<-blouch_simXYOU.mvSlouch.setup(trdata,num.direct=0,num.random=1,num.ME.direct=0,num.ME.random=0,slope,hl,vy,vY0,vX0,theta,mc=FALSE,sim.errors=FALSE) #Adaptive Nodel no ME

---
title: "Blouch ms Cervid Analysis SBR1 - Example"
output: html_notebook
editor_options: 
  chunk_output_type: console
---

Code for testing versions of Blouch using Cervidae (46 sp) tree
Also code for Cervidae
...
cervid.dataset<-read.csv("/Users/markgrabowski/Documents/Academic/Research/Current Projects/Blouch project/R1/Updated Dataset/Cervid Data/updated_mean_dat_no_OL_MG.csv")
...
#names.adaptive.traits<-c("mc.male_logmass","mc.female_logmass")
#names.direct.traits<-c("mc.male_logmass","mc.female_logmass")
name.response.me.trait<-c("me.log_ant_vol")
names.adaptive.me.traits<-c("me.log_psl")
names.direct.me.traits<-c("me.log_psl")

#names.direct.me.traits<-c("me.log_male_mass","me.log_female_mass")
#names.adaptive.me.traits<-c("me.log_male_mass","me.log_female_mass")
#Data must be logged before entry into blouchOU.setup

#stan_data<-blouchOU.setup.mv(cervid.trdata,name.response.trait,names.direct.traits,names.adaptive.traits=NULL,name.response.me.trait,names.direct.me.traits,names.adaptive.me.traits=NULL)
...
stanc("/Users/markgrabowski/Documents/Academic/Research/Current Projects/Blouch project/R1/Code/Stan Code/R1/blouchOU_test.stan")

stan_model <- stan_model("blouchOU_test.stan")

fit.adaptive.test<- rstan::sampling(object = stan_model,data = stan_data,chains = 2,iter = 2000,control=list(adapt_delta=0.99))

print(fit.adaptive.test,pars = c("a","hl","alpha","alpha_bar","beta","beta_e","vy","r_squared","sigma2_y"))
...
Plotting
```{r}
stan_dens(fit.adaptive.test,pars = c("a","hl","alpha_bar","beta","beta_e","vy","r_squared","sigma2_y"))


#For downstream analysis and plots
ext.fit.adaptive.test <- rstan::extract(fit.adaptive.test)

library(ggplot2)
hl<-ext.fit.adaptive.test$hl
vy<-ext.fit.adaptive.test$vy
beta<-ext.fit.adaptive.test$beta
sigma2_y<-ext.fit.adaptive.test$sigma2_y

new.data<-data.frame(hl,vy,beta,sigma2_y)
...
pairs(fit.adaptive.test,pars=c("hl","beta","vy","sigma2_y"))

```


Official version code
```{r}
setwd("/Users/markgrabowski/Documents/Academic/Research/Current Projects/Blouch project/R1/Code/Stan Code/R1")
...
fit.adaptive<- rstan::sampling(object = stan_model,data = stan_data,chains = 1,iter = 2000,control=list(adapt_delta=0.95))

print(fit.adaptive,pars = c("a","hl","beta","beta_e","vy","r_squared","sigma2_y"))
...
lml.fit.test<-bridge_sampler(fit.adaptive.test,silent=TRUE)
bridgesampling::bf(lml.fit.test, lml.fit.adaptive)


```


Plotting Joint Posterior to look for correlations
```{r}
library(ggplot2)
hl<-ext.fit.adaptive$hl
vy<-ext.fit.adaptive$vy
beta<-ext.fit.adaptive$beta[,2]
sigma2_y<-ext.fit.adaptive$sigma2_y
...
pairs(fit.adaptive,pars=c("hl","beta","vy","sigma2_y"))
...
slouch.OU<-slouch.fit(phy = cervid.trdata$phy,
                              ...
                              random.cov = cervid.trdata$dat$mc.log_psl,
...
Direct model of evolution
```{r}
stan_data<-blouchOU.setup.mv(cervid.trdata,name.response.trait,names.direct.traits,names.random.traits=NULL,name.response.me.trait,names.direct.me.traits,names.random.me.traits=NULL)
...
```{r}
library(ggplot2)
hl<-ext.fit.direct$hl
vy<-ext.fit.direct$vy
beta<-ext.fit.direct$beta[,2]
sigma2_y<-ext.fit.direct$sigma2_y
...
slouch.OU<-slouch.fit(phy = cervid.trdata$phy,
                              species = cervid.trdata$phy$tip.label,
                              response = cervid.trdata$dat$log_ant_vol,
                              direct.cov = cbind(mc.male_logmass=cervid.trdata$dat$mc.male_logmass,mc.female_logmass=cervid.trdata$dat$mc.female_logmass),
                              mv.response = cervid.trdata$dat$me.log_ant_vol,
                              mv.direct.cov = cbind(me.log_male_mass=cervid.trdata$dat$me.log_male_mass,me.log_female_mass=cervid.trdata$dat$me.log_female_mass),
...
source("/Users/markgrabowski/Documents/Academic/Research/Current Projects/Blouch project/R1/Code/R Setup Code/R1/blouchOUReg.setup.mv_v1_1.R")
...
stan_data<-blouchOUReg.setup.mv(cervid.trdata,name.response.trait,names.direct.traits,names.adaptive.traits=NULL,name.response.me.trait,names.direct.me.traits,names.adaptive.me.traits=NULL,names.fixed.fact)
...
```{r}
setwd("/Users/markgrabowski/Documents/Academic/Research/Current Projects/Blouch project/R1/Code/Stan Code/R1")
stanc("/Users/markgrabowski/Documents/Academic/Research/Current Projects/Blouch project/R1/Code/Stan Code/R1/blouchOUReg_v1_1.stan")
...
fit.direct<- rstan::sampling(object = stan_model,data = stan_constraint_data,chains = 2,iter = 4000,control=list(adapt_delta=0.95),show_messages=FALSE)

---
title: "Empirical Example from Grabowski (in revision)"
...
ggplot2::ggsave("/Users/markgrabowski/Documents/Academic/Research/Current Projects/Blouch Project - SBR2/Blouch ms/R2/Figures/Fig1.pdf", plot = dag, width=7, height=5 )
...
#cervidae.tree<-ape::read.nexus("/Users/markgrabowski/Documents/Academic/Research/Current Projects/Blouch Project - SBR2/Blouch project/blouch-testing/Sharable Data/cervidae_renamed.tre")
#antler.data<-read.csv("/Users/markgrabowski/Documents/Academic/Research/Current Projects/Blouch Project - SBR2/Blouch project/blouch-testing/Sharable Data/antler_dataset.csv")
```
...
#reconstructed.BGS <- ape::ace(as.factor(cervid.trdata.BGS$dat$BGS), cervid.trdata.BGS$phy, type = "d")
...
#FigS1b.plot<-recordPlot()

pdf("/Users/markgrabowski/Documents/Academic/Research/Current Projects/Blouch Project - SBR2/Blouch ms/R2/Figures/FigS1b.pdf", width=5, height=7 )
plot(cervid.trdata.BGS$phy,edge.color = reg.colors[edge.regimes], edge.width = 1, cex = 0.5)
dev.off()
```
...
Below are the priors used for this first analysis. See Grabowski (in revision) for more on setting these priors.
```{r}
############################################################################################################
#Set Priors
...
The first line below combines the existing trdata file from make.trdata which has regime info for the tips with the X and Y values and their errors. We will use the helper function blouch.reg.direct.prep() to setup the dat object for Stan. This function and the other helper functions included with *Blouch* require trdata files, and then the names of the columns that contain Y and (sometimes depending on the model) X data and error data. "Z_direct" is the number of predictors, with "BGS" the name of the column where the tip regime data is located. The rest are the priors we defined above. See help info for each function and other articles on github.com for functionality.

```{r}
############################################################################################################
#Blouch code to prep data for Adaptive Regimes + Direct Effect Model
#dat<-blouch:::blouch.reg.direct.prep(trdata,"Y_obs","Y_error","X_obs","X_error",Z_direct=1,"BGS",hl.prior,vy.prior,optima.prior,beta.prior)
...
Lets look at the Prior versus Posterior plots of the paramter estimates for the multi-optima direct effect model first.
...
Let's look at the Multi-optima Direct Effect Model First
```{r}
########################################################################################################
#Varying intercepts
post<-post.basic
...
Now lets run the multi-optima adaptive model with varying effects. This will allow our intercepts (optima) and slopes to vary with the regimes, but does not allow for multilevel modeling. We will use the same priors and the sane data formatting step above to produce our "dat" file.
```{r}
########################################################################################################
...
ggplot2::ggsave("/Users/markgrabowski/Documents/Academic/Research/Current Projects/Blouch Project - SBR2/Blouch ms/R2/Figures/Fig3.pdf", plot = fig3, width=9, height=7 )
...
Let's combine all four plots into one plot
```{r}
fig3<-fig3+ggplot2::theme(legend.position="none")
fig<-ggpubr::ggarrange(hl.plot, vy.plot,covariance.plot,fig3,ncol=2,nrow=2, labels = c("A)","B)","C)","D)"),common.legend = TRUE,legend="top")
...
The only new prior is on sigma
```{r}
...
############################################################################################################
#Blouch code to prep data for Adaptive Regimes + Direct Effect Model
#dat<-blouch:::blouch.reg.direct.mlm.prep(trdata,"Y_obs","Y_error","X_obs","X_error",Z_direct=1,"BGS",hl.prior,vy.prior,optima.prior,beta.prior,sigma.prior)
...
Next we will run the multilevel multi-optima direct effect model with varying intercepts.
```{r}
########################################################################################################
...
```{r Reg_direct_mlm}
########################################################################################################
#Multilevel model - multilevel optima with direct effects predictor, non-centered
...
Now let's run the Multilevel Multi-optima Direct Effects Model with Varying Effects. This will allow our intercepts (optima) and slopes to vary with the regimes. As a multilevel model, information can be shared across the regimes, which can produce more accurate parameter estimates.
...
rstan::traceplot(fit.reg.direct.mlm.ve.nc,pars = c("hl","vy","optima_bar","beta_bar","Rho[1,2]","sigma","optima","beta"))
...
plot.emp.mlm.ve<-rstan::traceplot(fit.reg.direct.mlm.ve.nc,pars = c("hl","vy","optima_bar","beta_bar","Rho[1,2]","sigma","optima","beta"))
plot.emp.vi<-rstan::traceplot(fit.reg.direct,pars = c("hl","vy","optima","beta"))
plot.emp.mlm.vi<-rstan::traceplot(fit.reg.direct.mlm.vi.nc,pars = c("hl","vy","optima","optima_bar","beta","sigma"))
...
Calculate direct causal effect of moving from smaller to larger breeding groups size - make all species have larger group sizes and all species have smaller group sizes and look at the contrast in antler size
```{r}
total.sp<-dat$N
sim.log.PSL<-seq(from=min(dat$X_obs),to=max(dat$X_obs),length.out=100)
regime<-1
optimas<-data.frame(post$optima)
betas<-data.frame(post$beta)

sim.log.ant.vol<-sapply(sim.log.PSL,function(log.PSL){
  rnorm(
    n<-length(post.ve$hl),
    mean = optimas[,regime]+log.PSL*betas[,regime],
    sd=post.ve$sigma2_y
    )
})

bigger<-rnorm(1000,post.ve$mu[])
smaller

plot(colMeans(sim.log.ant.vol)~sim.log.PSL)

with(post.ve,sapply(1:total.sp))

reg1<-sim.log.ant.vol
reg2<-sim.log.ant.vol
...
#ggplot2::ggsave("/Users/markgrabowski/Documents/Academic/Research/Current Projects/Blouch Project - SBR2/Blouch ms/R2/Latex Documents/Figures/Model Checking/Fig_PPC_Fig1.pdf",
...
ggplot2::ggsave("/Users/markgrabowski/Documents/Academic/Research/Current Projects/Blouch Project - SBR2/Blouch ms/R2/Figures/FigS5b.pdf", plot = fig, width=9, height=5)

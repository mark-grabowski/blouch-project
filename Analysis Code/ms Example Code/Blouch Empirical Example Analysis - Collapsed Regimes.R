...
########################################################################################################
#Collapsing BGS >3 to single regime
reconstructed.BGS <- ace(as.factor(cervid.trdata.BGS$dat$BGS.gr3B), cervid.trdata.BGS$phy, type = "d")

#Frugivore Diet Categories, group size categories, sociality
internal.regimes.BGS.gr3B<- apply(reconstructed.BGS$lik.anc, 
                                  1, 
...
cervid.trdata.BGS$phy$node.label<-as.factor(internal.regimes.BGS.gr3B)

#Check if manual setting code worked
shifts.total<-unlist(list(as.factor(cervid.trdata.BGS$dat$BGS.gr3B),factor(internal.regimes.BGS.gr3B)))
...
regimes_tip <- cervid.trdata.BGS$dat$BGS.gr3B
...
dat<-blouch.reg.direct.prep(trdata,"Y_obs","Y_error","X_obs","X_error","BGS.gr3B")
...
slope.plot<-ggplot()+  
  geom_point(data=df,aes(y=Y,x=X,color=cervid.trdata.BGS$dat$BGS.gr3B))+
...
fit.reg.direct.mlm.ve.nc.gr3B<- rstan::sampling(object = stan_model,data = dat,chains = 2,cores=2,iter =4000,control = list(adapt_delta = 0.99))
print(fit.reg.direct.mlm.ve.nc.gr3B,pars = c("hl","vy","optima_bar","beta_bar","Rho","sigma","optima","beta","Z"))
plot(precis(fit.reg.direct.mlm.ve.nc.gr3B,depth=3,pars = c("hl","vy","optima_bar","beta_bar","Rho","sigma","optima","beta")))
post<-extract(fit.reg.direct.mlm.ve.nc.gr3B)
...
fit.reg.direct.ve.gr3B<- rstan::sampling(object = stan_model,data = dat,chains = 2,cores=2,iter =4000)
print(fit.reg.direct.ve.gr3B,pars = c("hl","vy","optima","beta"))
plot(precis(fit.reg.direct.ve.gr3B,depth=3,pars = c("hl","vy","optima","beta")))
post<-extract(fit.reg.direct.ve.gr3B)
...
#First run
fit.reg.direct.mlm.vi.gr3B<- rstan::sampling(object = stan_model,data = dat,chains = 2,iter =4000,cores=2)
print(fit.reg.direct.mlm.vi.gr3B,pars = c("hl","vy","optima","optima_bar","beta","sigma"))
plot(precis(fit.reg.direct.mlm.vi.gr3B,depth=2,pars = c("hl","vy","optima","optima_bar","sigma")))

post<-extract(fit.reg.direct.mlm.vi.gr3B)
########################################################################################################
...
#First run
fit.reg.direct.mlm.vi.nc.gr3B<- rstan::sampling(object = stan_model,data = dat,chains = 2,iter =4000,cores=2,control = list(adapt_delta = 0.99))
print(fit.reg.direct.mlm.vi.nc.gr3B,pars = c("hl","vy","optima","optima_bar","beta","sigma"))
plot(precis(fit.reg.direct.mlm.vi.nc.gr3B,depth=2,pars = c("hl","vy","optima","optima_bar","sigma")))

post<-extract(fit.reg.direct.mlm.vi.nc.gr3B)
########################################################################################################
...
fit.reg.direct.gr3B<- rstan::sampling(object = stan_model,data = dat,chains = 2,iter =4000,cores=2)

print(fit.reg.direct.gr3B,pars = c("hl","vy","optima","beta"))
post<-extract(fit.reg.direct.gr3B)
########################################################################################################
#Compare slopes
...
slope.plot.1<-ggplot()+  
  ...
  #ggtitle("Prior vs. Posterior for Intercept and Slope")+
  ylab("log Antler Volume (l)") + xlab("log Posterior Skull Length (cm)")+
  scale_color_manual(name="Breeding Group Size",values=mypal,labels=c('1-2', '>3'))
#scale_color_discrete(name="Breeding Group Size",values=mypal,labels=c('1-2', '3-5', '>5'))+
#scale_color_aaas()
...
############################################################################################################
#Two regimes
loo_mlm_ve_nc.gr3B <- loo(fit.reg.direct.mlm.ve.nc.gr3B, save_psis = TRUE)
print(loo_mlm_ve_nc.gr3B)
plot(loo_mlm_ve_nc.gr3B)
plot(loo_mlm_ve_nc.gr3B,label_points=TRUE)

#library(loo) #Non-mlm varying effects model
loo_ve.gr3B <- loo(fit.reg.direct.ve.gr3B, save_psis = TRUE)
print(loo_ve.gr3B)
plot(loo_ve.gr3B)
plot(loo_ve.gr3B,label_points=TRUE)

#library(loo) #MLM Varying intercepts model
loo_mlm_vi_nc.gr3B <- loo(fit.reg.direct.mlm.vi.nc.gr3B, save_psis = TRUE)
print(loo_mlm_vi_nc.gr3B)
plot(loo_mlm_vi_nc.gr3B)
plot(loo_mlm_vi_nc.gr3B,label_points=TRUE)

#library(loo) #Basic model
loo_basic.gr3B <- loo(fit.reg.direct.gr3B, save_psis = TRUE)
print(loo_basic.gr3B)
plot(loo_basic.gr3B)
plot(loo_basic.gr3B,label_points=TRUE)

loo_compare(loo_mlm_ve_nc.gr3B, loo_ve.gr3B, loo_mlm_vi_nc.gr3B, loo_basic.gr3B)
#elpd_diff se_diff
#model2  0.0       0.0   
#model1 -1.1       0.9   
#model4 -2.2       3.5   
#model3 -2.3       3.9  
#95% interval of difference
-1.1+c(-1,1)*0.9*1.96
-2.3+c(-1,1)*3.9*1.96

loo_compare(loo_ve.gr3B,loo_ve)
...
bridgesampling::bf(lml.fit.reg.direct.mlm.ve.nc, lml.fit.reg.direct.mlm.vi.nc)
bridgesampling::bf(lml.fit.reg.direct.mlm.ve.nc, lml.fit.reg.direct.ve)
bridgesampling::bf(lml.fit.reg.direct.mlm.ve.nc, lml.fit.reg.direct)
bridgesampling::bf(lml.fit.reg.direct.ve, lml.fit.reg.direct)
############################################################################################################
#Trankplots/Traceplots
...
traceplot(fit.reg.direct.vs,pars = c("hl","vy","optima_beta","beta"))
############################################################################################################

...
```
For the purposes of illustrating the software, we will use a dataset of ruminant neocortices bundled with the Slouch package and a corresponding phylogenetic tree (Toljagić et al. 2017). First, we will organize the neocortex data and associated annotation data.
```{r load data}
...
Here we will create a R object called stan_data which has all the data we are sending to Blouch in the Stan format. This includes data setups for both the direct effect (constraint) and adaptive models we will run below. These are further discussed in Hansen et al. (2008) and Grabowski et al. (2016).


First we will run Blouch including Measurement Error. Measurement error is estimation error in species averages following Hansen and Bartoszek (2012), and Blouch can handle measurement error in both the predictor and response variables. Following Grabowski et al. (2016), measurement error is the average of the sample variances, weighted by the sample size of each species. This procedure allows for the species with larger sample size to be weighted more, and this average measurement variance is then divided by the sample size of each species. See Grabowski et al (2016) for further discussion. 
```{r Blouch setup}
par(mfrow = c(1,1))
...
#Not including Measurement Error
#names.traits<-c("neocortex_area_mm2_log_mean","braincentered",NA,NA)

#Including Measurement Error, where the last two named traits are the measurement error of the first two traits
#Setup: names.traits = c(response, predictor, me.response, me.predictor)
...
stan_data<-blouch::blouchOU.setup.v1(ruminant.trdata,names.traits)
...
```{r}
setwd("/Users/markgrabowski/Documents/Academic/Research/Current Projects/blouch/R/R Functions/Blouch Functions")
stanc("/Users/markgrabowski/Documents/Academic/Research/Current Projects/blouch/R/R Functions/Blouch Functions/blouchBM_v1.stan")
stan_model <- stan_model("blouchBM_v1.stan")

fit.BM<- rstan::sampling(object = stan_model,data = stan_constraint_data,chains = 2,iter = 4000,control=list(adapt_delta=0.95))
...
```{r}
ggplot(data=ruminant.trdata$dat,aes(y=neocortex_area_mm2_log_mean,x=braincentered))+
  #geom_abline(intercept=intercept_test,slope=slope_test,alpha=0.25)+
